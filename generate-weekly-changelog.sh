...
SOURCE_WEEKLY=../jenkins.io/content/_data/changelogs/weekly.yml
...
if [ ! -s /tmp/new-changelog-entry.yaml ]; then
  echo "ERROR: generated changelog entry is empty; refusing to update weekly.yml" >&2
  exit 1
fi
...
../core-changelog-generator/replace-changelog-entry.py "${NEXT_JENKINS_VERSION}" "${SOURCE_WEEKLY}" /tmp/new-changelog-entry.yaml \
...
SOURCE_SIZE=$(wc -c < "${SOURCE_WEEKLY}" | tr -d ' ')
MERGED_SIZE=$(wc -c < /tmp/weekly.yaml | tr -d ' ')
# A failed merge used to leave only the header/footer comments (~300 bytes).
# Refuse to overwrite the historical changelog if the merged file shrank.
if [ "${MERGED_SIZE}" -lt $((SOURCE_SIZE * 9 / 10)) ]; then
  echo "ERROR: merged weekly.yaml is ${MERGED_SIZE} bytes but source weekly.yml is ${SOURCE_SIZE} bytes; refusing to overwrite" >&2
  exit 1
fi

cp /tmp/weekly.yaml "${SOURCE_WEEKLY}"
